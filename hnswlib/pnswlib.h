...
class ProbabilisticNSW : public HierarchicalNSW<dist_t> {
 public:
  ProbabilisticNSW(SpaceInterface<dist_t> *s) : HierarchicalNSW<dist_t>(s) {}

  ProbabilisticNSW(SpaceInterface<dist_t> *s,
      const std::string &location, bool nmslib = false, size_t max_elements=0) :
      HierarchicalNSW<dist_t>(s, location, nmslib, max_elements) {}

  ProbabilisticNSW(SpaceInterface<dist_t> *s, size_t max_elements,
  ...
  ~ ProbabilisticNSW() = default;

  using PriorityQueue = std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, typename HierarchicalNSW<dist_t>::CompareByFirst>;
  PriorityQueue searchBaseLayerSTProbabilistic(tableint ep_id, const void *data_point, size_t ef)  {
...
*/
  ...
  std::priority_queue<std::pair<dist_t, labeltype >> searchKnn(const void *query_data, size_t k)  {
    ...
    PriorityQueue top_candidates = searchBaseLayerSTProbabilistic(
